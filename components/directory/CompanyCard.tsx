...
export function CompanyCard({ company }: CompanyCardProps) {
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <h3
              ...
              className="font-heading font-bold text-[#0F172A] text-xl leading-tight truncate"
            >
              {company.name}
            </h3>
      ...
      {/* Location */}
      <div className="relative mt-4 flex items-center gap-1.5 text-sm text-[#475569]">
        <MapPin className="w-4 h-4" style={{ color: style.color }} />
      ...
      {/* Actions */}
      <div className="relative mt-5 flex flex-wrap items-center gap-2 pt-4 border-t border-[#F1F5F9]">
        {company.clutchProfileUrl ? (
          <a
            href={company.clutchProfileUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold text-white transition-all hover:-translate-y-0.5"
            style={{
              background: style.gradient,
              boxShadow: `0 8px 24px -8px ${style.color}80`,
            }}
            aria-label={`View ${company.name} profile`}
          >
            View Profile <ArrowUpRight className="w-3.5 h-3.5" />
          </a>
        ) : company.source !== "Member" ? (
          <span
        ...
        {company.websiteUrl && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 rounded-full border border-[#CBD5E1] px-4 py-2 text-sm font-semibold text-[#0F172A] transition-colors"
            style={
              {
                ["--hover-color" as string]: style.color,
        ...
        <span className="ml-auto text-[11px] uppercase tracking-wider text-[#94A3B8]">
          {company.source === "Member" ? "UPTECH Verified Member" : `Data source: ${company.source}`}
        </span>
      </div>
    </article>
  );
