...
function Row({ chips, reverse, duration }: { chips: Chip[]; reverse?: boolean; duration: number }) {
  ...
  return (
    <div className="relative overflow-hidden">
      {/* edge fade masks */}
      <div className="pointer-events-none absolute inset-y-0 left-0 w-24 z-10 bg-gradient-to-r from-[#0B1C2D] to-transparent" />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-24 z-10 bg-gradient-to-l from-[#0B1C2D] to-transparent" />
...
export function TechEcosystem({ companies }: TechEcosystemProps) {
  ...
  return (
    <section
      ...
      style={{
        background:
          "radial-gradient(900px circle at 12% 50%, rgba(31,79,216,0.18), transparent 60%), radial-gradient(900px circle at 88% 50%, rgba(1,169,92,0.16), transparent 60%), linear-gradient(180deg, #0B1C2D 0%, #0E2138 100%)",
      }}
    >
      {/* Dot grid */}
      <div
