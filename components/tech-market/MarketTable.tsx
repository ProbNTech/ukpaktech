...
export function MarketTable({ sectors, title }: MarketTableProps) {
  ...
  return (
    <div>
      {title && (
        <h3 className="font-heading font-bold text-xl text-[#1a2b5e] mb-4">
          {title}
        </h3>
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {sectors.map((sector, i) => (
              ...
              >
                <td className="px-5 py-3.5 font-semibold text-[#1a2b5e] whitespace-nowrap">
                  {sector.sector}
                </td>
                <td className="px-5 py-3.5 text-[#3D4152]">
                  {sector.currentMarket}
                </td>
                <td className="px-5 py-3.5 text-[#22C55E] font-medium">
                  {sector.forecast}
                </td>
      ...
      <div className="md:hidden space-y-3">
        {sectors.map((sector, i) => (
          ...
          >
            <h4 className="font-semibold text-[#1a2b5e] mb-2">
              {sector.sector}
            </h4>
