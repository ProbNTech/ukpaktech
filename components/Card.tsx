...
export function Card({ children, className = "", hover = false, variant = "light" }: CardProps) {
  ...
  const hoverClass = hover ? "hover:-translate-y-1 hover:shadow-lg hover:border-[#2563EB]/20 cursor-pointer" : "";
    ...
    >
      {hover && isHovering && (
        <div
          ...
          style={{
            background: `radial-gradient(circle 250px at ${mousePos.x}% ${mousePos.y}%, rgba(37,99,235,0.06), transparent)`,
          }}
        />
      )}
      {children}
