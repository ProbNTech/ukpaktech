  ...
  return (
    <main>
                ...
                {country.highlights.map((h, i) => (
                  <AnimatedSection key={h.label} delay={i * 0.06}>
                    <div className="bg-white rounded-xl border border-[#e2e0dc] p-4 text-center h-full">
                      <p className="text-lg sm:text-xl font-extrabold text-[#2563EB] mb-1">
                        {h.value}
                      </p>
                ...
                {country.detailedSections.map((section, i) => (
                  <AnimatedSection key={section.title} delay={i * 0.08}>
                    <div className="bg-white rounded-xl border border-[#e2e0dc] p-5 h-full">
                      <h3 className="font-semibold text-[#1a2b5e] mb-2">
                        {section.title}
                      </h3>
              ...
              <AnimatedSection>
                <div className="bg-white rounded-xl border border-[#e2e0dc] p-6 h-full">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-heading font-bold text-lg text-[#1a2b5e]">
                      Growth Drivers
                    </h3>
                  </div>
            ...
            <div className="flex flex-wrap gap-3">
              {country.highDemandSectors.map((sector, i) => (
                <AnimatedSection key={sector} delay={i * 0.05}>
                  <span className="inline-block px-4 py-2 rounded-full bg-[#f0f4ff] text-[#2563EB] font-medium text-sm border border-[#2563EB]/15">
                    {sector}
                  </span>
                </AnimatedSection>
              ))}
              ...
              {country.opportunitiesForPakistan.map((opp, i) => (
                <AnimatedSection key={opp.title} delay={i * 0.08}>
                  <div className="bg-white rounded-xl border border-[#e2e0dc] p-5 h-full hover:shadow-md transition-shadow">
                    ...
                    <h4 className="font-semibold text-[#1a2b5e] mb-1">
                      {opp.title}
                    </h4>
          ...
          <div className="grid grid-cols-3 gap-4 items-stretch">
            ...
            {prevCountry ? (
              <Link
                ...
                className="group flex items-center gap-3 bg-white rounded-xl border border-[#e2e0dc] p-4 sm:p-5 hover:border-[#2563EB]/30 hover:shadow-md transition-all"
              >
                <ArrowLeft className="w-5 h-5 text-[#2563EB] flex-shrink-0 group-hover:-translate-x-1 transition-transform" />
                <div className="min-w-0">
                  <p className="text-xs text-[#6b7280] mb-0.5">Previous</p>
                  <p className="font-semibold text-[#1a2b5e] text-sm truncate">
            ...
            <Link
              ...
              className="flex items-center justify-center bg-[#1a2b5e] rounded-xl p-4 sm:p-5 text-white font-semibold text-sm hover:bg-[#2563EB] transition-colors text-center"
            >
              All Markets
            </Link>
            ...
            {nextCountry ? (
              <Link
                ...
                className="group flex items-center justify-end gap-3 bg-white rounded-xl border border-[#e2e0dc] p-4 sm:p-5 hover:border-[#2563EB]/30 hover:shadow-md transition-all text-right"
              >
                <div className="min-w-0">
                ...
                <ArrowRight className="w-5 h-5 text-[#2563EB] flex-shrink-0 group-hover:translate-x-1 transition-transform" />
              </Link>
            ) : (
              <div />
            )}
      ...
      <GlobalCTA
        ...
        subtitle={`Let UPTECH help your company access opportunities in ${country.name}'s technology sector.`}
        primaryButtonText="Contact Us"
        primaryButtonLink="/contact"
        secondaryButtonText="Join UPTECH"
